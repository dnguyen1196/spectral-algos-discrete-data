...
class GLRM(object):
    ...
    def __init__(self, A, loss, regX, regY, k, missing_list = None, converge = None, scale=True, X0 = None, Y0 = None):
        ...
        self._initialize_probs(A, k, missing_list, regX, regY, X0, Y0)
    ...
    def fit(self, max_iters=100, eps=1e-2, use_indirect=False, warm_start=False, solver=cp.SCS):
        ...
        while not self.converge.d():
            objX = pX.solve(solver=cp.SCS, eps=eps, max_iters=max_iters,
                    use_indirect=use_indirect, warm_start=warm_start) # [0]
    ...
    def _initialize_probs(self, A, k, missing_list, regX, regY, X0, Y0):
        ...
        # cvxpy problems
        Xv, Yp = cp.Variable((m,k)), [cp.Parameter((k+1,ns[0]))]
        Xp, Yv = cp.Parameter((m,k+1)), [cp.Variable((k+1,ns[0]))]


        Xp.value = copy(X0)
        Yp[0].value = copy(Y0[0])
